...
class TemporalServiceTests(unittest.TestCase):
    def test_wall_monotonic_and_world_time_remain_distinct(self) -> None:
        ...
        self.assertEqual(
            service.sleep_interval(FRIDAY, woke_at=MONDAY).elapsed_wall_time, timedelta(hours=65)
        )
...
class FreshnessAndReconciliationTests(unittest.TestCase):
    ...
    def test_one_hundred_sources_refresh_only_four_decision_relevant_sources(self) -> None:
        states = tuple(
            source_state(
                f"source-{index:03d}",
                relevance=1.0 if index < 4 else 0.1,
                sensitivity=1.0 if index < 4 else 0.1,
            )
        ...
        plan = WakeReconciler().plan(
            states,
            elapsed_wall_time=timedelta(hours=65),
    ...
    def test_orientation_barrier_is_shadow_only_and_exposes_missing_prerequisites(self) -> None:
        coverage = AwarenessCoverage.from_states((source_state("calendar"),))
